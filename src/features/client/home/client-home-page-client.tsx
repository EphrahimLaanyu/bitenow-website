  ...
  return (
    <div className="space-y-12 pb-24 md:pb-20">
      ...
      <div id="hotels-section" className="space-y-6 md:space-y-8 scroll-mt-24">
        ...
        {/* ERROR STATE */}
        {error ? (
          <div className={cn("flex flex-col gap-4 rounded-[1.5rem] border p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between", needsSignIn ? "signin-notice" : "border-amber-200 bg-amber-50")}>
            <div className="flex items-start gap-3"><div className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl", needsSignIn ? "bg-[#f97316] text-white shadow-[0_8px_20px_rgba(249,115,22,0.25)]" : "bg-amber-100 text-amber-800")}>{needsSignIn ? <LogIn aria-hidden size={19} /> : <Search aria-hidden size={19} />}</div><div><p className={cn("font-bold", needsSignIn ? "text-[var(--foreground)]" : "text-amber-900")}>{needsSignIn ? "Sign in to see hotels available to your stay" : "We couldn’t load hotels"}</p><p className={cn("mt-1 text-sm leading-relaxed", needsSignIn ? "text-[var(--muted)]" : "text-amber-800")}>{needsSignIn ? "Your saved hotels, menus, and live availability will appear here after you sign in." : error}</p></div></div>
            {needsSignIn ? (
              <Link
